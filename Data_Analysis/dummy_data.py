...
for group in dummy_data.keys():
    for user in dummy_data[group]['user_id']:
        for main_category in dummy_data[group]['main_category']:
            raw_dummy_data.append([user, main_category, random.randrange(2, 5)])
        for sub1_category in dummy_data[group]['sub1_category']:
            raw_dummy_data.append([user, sub1_category, random.randrange(1, 4)])
        for sub2_category in dummy_data[group]['sub2_category']:
            raw_dummy_data.append([user, sub2_category, random.randrange(0, 3)])
        for etc_category in dummy_data[group]['etc_category']:
            raw_dummy_data.append([user, etc_category, random.randrange(0, 2)])
...
# print(pd.pivot(data=df_new, index='user', columns='category', values='count'))
